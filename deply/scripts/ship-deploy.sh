...
helm lint "${REPO_ROOT}/deply/helm/frontend-bff"
...
ssh ${SSH_OPTS} "${DEPLOY_TARGET}" "mkdir -p '${DEPLOY_PATH}'"

echo "Copying source and image archives to ${DEPLOY_TARGET}"
scp ${SSH_OPTS} "${TEMP_DIR}/${SOURCE_ARCHIVE_NAME}" "${TEMP_DIR}/${IMAGE_ARCHIVE_NAME}" "${DEPLOY_TARGET}:${DEPLOY_PATH}/"
...
ssh ${SSH_OPTS} "${DEPLOY_TARGET}" \
  "cd '${DEPLOY_PATH}' && \
   ...
   IMAGE_TAG='${IMAGE_TAG}' \
   ./deply/scripts/remote-deploy.sh"
